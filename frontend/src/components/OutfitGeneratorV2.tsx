...
export default function OutfitGeneratorV2({
  ...
  const getFinalValues = () => ({
    occasion:
      customOccasion.trim() ||
      OCCASION_PRESETS.find((p) => p.id === selectedOccasion)?.label ||
      "",
    weather:
      customWeather.trim() ||
      (selectedWeather
  ...
  const handleRestoreHistory = (entry: HistoryEntry) => {
    ...
    // if (entry.request_details.occasion)
    //   setSelectedOccasion(entry.request_details.occasion);
    setCustomOccasion(entry.request_details.occasion || "");
    setCustomWeather(entry.request_details.weather || "");
    setFreeText(entry.request_details.description || "");
    setSelectedStyle(entry.request_details.style_id || "");
    console.log("Restored style ID:", entry.request_details);
    console.log(selectedStyle);
    console.log(getFinalValues());
                  ...
                  <div className="pt-4">
                    ...
                    <div className="flex items-center gap-4">
                      <input
                        ...
                        value={numOutfits}
                        onChange={(e) =>
                          setNumOutfits(parseInt(e.target.value))
                        }
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card p-6 rounded-2xl border shadow-sm">
              <div>
                ...
                <p className="text-muted-foreground mt-1">
                  Based on {selectedStyle ? "your style" : "your input"} and{" "}
                  {selectedItems.length} specific items, for the following
                  constraints:
                </p>
...
function OutfitResultCard({
  ...
  wardrobe: ClothingItem[];
  onSave: () => void;
  getColorHex: (colorOrSlug: string) => string;
}) {
          ...
          <div>
            ...
            <CardDescription className="mt-2 text-xs">
              {outfit.ai_generated_reasoning ||
                "Perfectly matched for your occasion."}
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="flex-1 p-0">
        <div className="divide-y">
          {items.map((item, i) => (
            ...
            >
              {/* if not item.icon_id else clothingicon */}
              {!item.icon_id && (
