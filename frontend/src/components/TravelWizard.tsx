...
const getPieceLabel = (item: ClothingItem) => {
  ...
  const fit = item.fit ? ` (${item.fit})` : "";
  const label = parts.join(" ") + fit;
  return label || "Item";
};
...
export default function TravelWizard({
  ...
  const [suitcaseResult, setSuitcaseResult] =
    useState<SuitcaseGenResponse | null>(null);
  ...
  const [viewingSuitcase, setViewingSuitcase] = useState<TravelSuitcase | null>(
    null
  );
  ...
  if (viewingSuitcase) {
    const suitcaseDuration =
      Math.ceil(
        (new Date(viewingSuitcase.end_date).getTime() -
          new Date(viewingSuitcase.start_date).getTime()) /
          (1000 * 60 * 60 * 24)
      ) + 1;
        ...
        <main className="flex-1 overflow-y-auto px-4 pb-8 space-y-6">
          <div>
            <h2 className="text-lg font-semibold">
              {viewingSuitcase.name || viewingSuitcase.destination}
            </h2>
            <p className="text-xs text-muted-foreground">
              {new Date(viewingSuitcase.start_date).toLocaleDateString()} -{" "}
              {new Date(viewingSuitcase.end_date).toLocaleDateString()} (
              {suitcaseDuration} days)
            </p>
          </div>
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="outline" className="text-xs">
                {viewingSuitcase.packing_list.length} items
              </Badge>
            </div>
          ...
          <div className="max-w-sm w-full text-center space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Travel Suitcase is available for Pro users. Get AI-powered
                packing lists that maximize outfit combinations while minimizing
                items.
              </p>
            </div>
          ...
          {savedSuitcases.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="text-sm font-medium text-muted-foreground">
                No saved suitcases yet
              </h3>
              <p className="text-xs text-muted-foreground mt-1">
                Create and save a trip to see it here
              </p>
              <Button
                variant="outline"
                size="sm"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {savedSuitcases.map((suitcase) => {
                const suitcaseDuration =
                  Math.ceil(
                    (new Date(suitcase.end_date).getTime() -
                      new Date(suitcase.start_date).getTime()) /
                      (1000 * 60 * 60 * 24)
                  ) + 1;
                return (
                  <Card
                  ...
                  >
                    <CardHeader className="pb-2">
                      ...
                      <CardDescription className="text-xs">
                        {new Date(suitcase.start_date).toLocaleDateString()} -{" "}
                        {new Date(suitcase.end_date).toLocaleDateString()}
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="text-xs text-muted-foreground">
                      <p>
                        {suitcaseDuration} days • {suitcase.packing_list.length}{" "}
                        items • {suitcase.daily_outfits.length} outfits
                      </p>
                    </CardContent>
                  </Card>
                );
        ...
        <>
          ...
          <div className="px-4 pb-2">
            <div className="flex items-center gap-2 text-[10px] uppercase tracking-wide text-muted-foreground">
              ...
              <span
                className={`px-2 py-1 rounded-full ${
                  step === 3 ? "bg-primary/10 text-primary" : "bg-muted"
                }`}
              >
                3. Suitcase
              </span>
                ...
                >
                  ...
                  {styles.length > 0 && (
                    <section className="space-y-3">
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        Select 1-3 styles to guide outfit creation. Multiple
                        styles add variety.
                      </p>
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {suitcaseResult.packing_list.length} items for{" "}
                        {duration} days
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-muted-foreground">
                            ...
                            {suitcaseResult.daily_outfits.length}
                          </span>
                          <Button
                            variant="outline"
                            size="icon"
                            ...
                            onClick={() =>
                              setCurrentOutfitIndex((prev) =>
                                Math.min(
                                  suitcaseResult.daily_outfits.length - 1,
                                  prev + 1
                                )
                              )
                            }
                ...
                <div className="flex flex-col text-[10px] text-muted-foreground">
                  {canGenerate ? (
                    <>
                      ...
                      <span>
                        {duration} days, {selectedOccasions.length} activities
                        {selectedStyles.length > 0
